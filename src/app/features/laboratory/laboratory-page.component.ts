...
type RejectTarget = { resultId: string; title: string; subtitle: string; fromReview: boolean };
type PendingReportOrder = LabOrder & { processingCount: number; verificationCount: number };
...
@Component({
    ...
    <main class="laboratory-page">
      ...
      @else {
        @switch(activeTab()){
          ...
          @case('verification'){
            <section class="panel verification-panel"><div class="panel-head"><div><p>Authorized review</p><h2>Verification queue</h2></div><span>Open every result before release</span></div><div class="cards">@for(v of verification();track v.resultId){<article class="verify-card review-card"><div><span class="test-code">{{v.sampleNumber}}</span><h3>{{v.patientName}}</h3><p>{{v.testName}} · Technician: {{v.technicianName||'-'}}</p><small>{{v.orderNumber}} · Submitted {{date(v.submittedAt)}}</small></div>@if(v.hasCritical){<mark class="danger">CRITICAL</mark>}<div class="actions"><button class="ac-btn ac-btn-secondary" (click)="reviewResult(v)"><span class="material-symbols-rounded">visibility</span>Review report</button><button (click)="openRejectResult(v)">Reject</button><button class="primary" (click)="verify(v)">Verify & release</button></div></article>}@empty{<div class="empty pretty-empty"><span class="material-symbols-rounded">fact_check</span><strong>No results await verification</strong><small>Submitted lab results will appear here for authorized review.</small></div>}</div></section>
          }
          @case('reports'){
            @if(pendingReportOrders().length){<section class="panel pending-report-panel"><div class="panel-head"><div><p>Report generation tracker</p><h2>Reports waiting to be completed</h2></div><span>Reports generate after every test in the order is verified</span></div><div class="pending-report-cards">@for(o of pendingReportOrders();track o.id){<article class="pending-report-card"><span class="material-symbols-rounded">{{o.verificationCount?'fact_check':'science'}}</span><div><div class="work-title"><strong>{{o.orderNumber}}</strong><mark [class.stat]="o.priority==='STAT'">{{o.priority}}</mark><b class="status" [ngClass]="statusClass(o.statusCode)">{{status(o.statusCode)}}</b></div><h3>{{o.patientName}}</h3><p>{{o.medicalRecordNo}} · {{o.itemCount}} ordered tests</p><small>{{reportBlockingText(o)}}</small></div><button class="ac-btn ac-btn-secondary" type="button" (click)="openReportBlocker(o)">{{o.verificationCount ? 'Open Verification' : 'Open Processing'}}</button></article>}</div></section>}
            <section class="panel"><div class="panel-head"><div><p>Released diagnostics</p><h2>Laboratory reports</h2></div><span>{{reports().length}} released report{{reports().length===1?'':'s'}}</span></div><div class="table"><div class="table-head report-grid"><span>Report</span><span>Patient</span><span>Order</span><span>Released</span><span>Actions</span></div>@for(r of reports();track r.id){<div class="table-row report-grid"><span><strong>{{r.reportNumber}}</strong><small>Version {{r.currentVersion}}</small></span><span>{{r.patientName}}<small>{{r.medicalRecordNo}}</small></span><span>{{r.orderNumber}}</span><span>{{date(r.releasedAt)}}</span><span><button (click)="download(r)"><span class="material-symbols-rounded">picture_as_pdf</span>PDF</button></span></div>}@empty{<div class="empty pretty-empty"><span class="material-symbols-rounded">description</span><strong>No released reports yet</strong><small>Once all tests in an order are verified, the report PDF will appear here.</small></div>}</div></section>
          }
          @case('critical'){
  ...
  `, `
    ...
    @media(max-width:650px){.laboratory-page{gap:12px}.page-head h1{font-size:24px}.head-actions .ac-btn{flex:1}.metric-grid,.processing-summary,.collection-summary{grid-template-columns:1fr}.toolbar,.panel-head{align-items:stretch;flex-direction:column}.panel-head input,.search-field,.barcode-field{min-width:0;max-width:none}.checks{grid-template-columns:1fr}.panel,.dialog{padding:14px}.dialog-overlay{padding:8px}.work-card,.collection-card,.barcode-card,.pending-report-card{grid-template-columns:1fr}.work-action{justify-content:stretch}.work-action .ac-btn,.barcode-card .ac-btn{width:100%;justify-content:center}}
  `]
})
...
function countByOrder(orderNumbers: string[]): Map<string, number> {
  const counts = new Map<string, number>();
  for (const orderNumber of orderNumbers) counts.set(orderNumber, (counts.get(orderNumber) || 0) + 1);
  return counts;
}
