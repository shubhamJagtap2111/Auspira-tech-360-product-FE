...
@Component({
  ...
  template: `
    <main class="auth-center-page">
      <form class="auth-card" (ngSubmit)="onVerify()">
        <span class="material-symbols-rounded icon">mark_email_read</span>
        <h2>{{ t('Auth.VerifyEmail.Title') }}</h2>
        <p>{{ t(verified() ? 'Auth.Messages.EmailVerificationSuccessful' : 'Auth.VerifyEmail.Description') }}</p>
        ...
        @if (!verified()) {
          ...
          <button class="primary" type="submit" [disabled]="loading()">
            {{ t(loading() ? 'Common.Actions.Verifying' : 'Auth.VerifyEmail.Submit') }}
          </button>
        }
...
export class VerifyEmailPageComponent {
  ...
  protected async onVerify(): Promise<void> {
    ...
    try {
      const response = await this.authService.verifyEmail({ email: this.email, token: this.token });
      this.verified.set(response.success);
