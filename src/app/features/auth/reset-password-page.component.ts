  ...
  template: `
    <main class="auth-center-page">
      <form class="auth-card" (ngSubmit)="onSubmit()">
        ...
        <p>{{ t(success() ? 'Auth.Messages.ResetPasswordSuccessful' : 'Auth.ResetPassword.Description') }}</p>
        ...
        @if (!success()) {
          ...
          <label>
            <span>{{ t('Auth.ResetPassword.Token.Label') }}</span>
            <input type="text" name="token" [(ngModel)]="token" [placeholder]="t('Auth.ResetPassword.Token.Placeholder')" required />
          </label>
          <label>
            <span>{{ t('Auth.ResetPassword.NewPassword.Label') }}</span>
            <input type="password" name="password" [(ngModel)]="newPassword" [placeholder]="t('Auth.Login.Password.Placeholder')" required />
          </label>
          <button class="primary" type="submit" [disabled]="loading()">
            {{ t(loading() ? 'Common.Actions.Updating' : 'Auth.ResetPassword.Submit') }}
          </button>
        }
...
export class ResetPasswordPageComponent {
  ...
  protected async onSubmit(): Promise<void> {
    ...
    try {
      const response = await this.authService.resetPassword({ email: this.email, token: this.token, newPassword: this.newPassword });
      this.success.set(response.success);
      this.errorKey.set(response.success ? null : response.message);
    } catch {
      this.errorKey.set('Auth.Errors.InvalidSecurityToken');
    } finally {
